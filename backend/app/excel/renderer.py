...
def _find_last_nonempty_row(
    ws: Worksheet, *, min_col: int = 1, max_col: int = 13
) -> int:
    """
...
def _merge_footnote_block_a_h(ws: Worksheet, start_row: int) -> None:
    ...
    """
    ws.merge_cells(
        start_row=start_row, start_column=1, end_row=start_row + 2, end_column=8
    )
...
def render_statements_sheet(
    ...
    for stmt in stmts_sorted:
        ...
        table_bottom = tail.total_row if tail.total_row is not None else tail.last_row
...
def _render_contract_header_at(ws: Worksheet, body: Any, *, start_row: int) -> int:
    ...
    # Категория (B..F merged + thick outer border)
    set_cell(ws, f"A{r_cat}", "Категория:", bold=True, size=13, h="left", v="center")
    ws.merge_cells(f"B{r_cat}:F{r_cat}")
    ...
    set_cell(
        ws, f"B{r_cat}", category_display, bold=True, size=13, h="center", v="center"
    )
    apply_thick_outer_border(ws, f"B{r_cat}", f"F{r_cat}")
    ...
    set_cell(
        ws, f"G{r_h13}", "с", size=11, h="center", v="center", fill=FILL_YELLOW_LIGHT
    )
    set_cell(
        ws, f"H{r_h13}", "по", size=11, h="center", v="center", fill=FILL_YELLOW_LIGHT
    )
    set_cell(
        ws, f"I{r_h13}", "дней", size=11, h="center", v="center", fill=FILL_YELLOW_LIGHT
    )
    ...
    for idx, col in enumerate(list("ABCDEFGHIJKLM"), start=1):
        set_cell(
            ws, f"{col}{r_nums}", idx, size=11, h="center", v="center", fill=FILL_GRAY
        )
...
def _render_sprav_block_sum(
    ...
    *,
    ...
    """
    set_cell(
        ws, f"J{sprav_start}", "СПРАВОЧНО", bold=True, size=13, h="left", v="center"
    )
    ...
    set_cell(
        ws, f"J{sprav_start+2}", "в т.ч. основной долг", size=13, h="left", v="center"
    )
    set_cell(
        ws,
        f"M{sprav_start+2}",
...
def _render_calc_rows_g_h_k_i(
    ...
    *,
    ...
    def _estimate_wrapped_lines_for_col(text: str, col_letter: str) -> int:
        if not text:
            return 1
        col_w = COLUMN_WIDTHS.get(col_letter) or float(
            ws.column_dimensions[col_letter].width or 20.0
        )
    ...
    def _distribute_height_over_block(
        block_start_row: int, block_end_row: int, text: str
    ) -> None:
    ...
    def close_block_and_subtotal(block_start_row: int, block_end_row: int) -> int:
        ...
        if lbl.strip().startswith(AA_PREFIX):
            ...
            ws[f"A{block_start_row}"].alignment = ws[
                f"A{block_start_row}"
            ].alignment.copy(wrap_text=True, vertical="center")
        ...
        set_cell(
            ws, f"B{sr}", "ИТОГО ПО ПЕРИОДУ", bold=True, size=10, h="right", v="center"
        )

        set_cell(
            ws,
            f"C{sr}",
            f"=SUM(C{block_start_row}:C{block_end_row})",
            bold=True,
            size=10,
            h="center",
            v="center",
        ...
        set_cell(
            ws,
            f"D{sr}",
            f"=SUM(D{block_start_row}:D{block_end_row})",
            bold=True,
            size=10,
            h="center",
            v="center",
        ...
        set_cell(
            ws,
            f"M{sr}",
            f"=SUM(M{block_start_row}:M{block_end_row})",
            bold=True,
            size=10,
            h="center",
            v="center",
    ...
    for row in rows:
        ...
        if is_charge_row:
            ...
            is_aa_block = bool(
                row.period_label
            ) and row.period_label.strip().startswith(AA_PREFIX)
        ...
        formula_txt = row.formula_text or (
            "6 * 9 * 10 * 11" if (has_overdue and row.fraction is not None) else ""
        )
    ...
    if subtotal_rows:
        ...
        set_cell(
            ws, f"B{total_row}", "ИТОГО", bold=True, size=10, h="center", v="center"
        )
        ...
        set_cell(
            ws, f"J{footer_start}", "Всего долг, руб.", size=10, h="left", v="center"
        )
        set_cell(
            ws,
            f"M{footer_start}",
        ...
        set_cell(
            ...
            v="center",
            num_fmt=MONEY_FMT,
            fill=FILL_TOTAL_YELLOW,
        )

        set_cell(
            ws,
            f"J{footer_start+2}",
            "ИТОГО, долг + неустойка, руб.",
            bold=True,
            size=10,
            h="left",
            v="center",
        ...
        set_cell(
            ws,
            f"M{footer_start+2}",
            f"=M{footer_start}+M{footer_start+1}",
            bold=True,
            size=10,
            h="center",
            v="center",
            num_fmt=MONEY_FMT,
            fill=FILL_TOTAL_LIME,
        )

        from openpyxl.styles import Border, Side

        thin = Side(style="thin", color="FF000000")
        ws[f"M{footer_start+2}"].border = Border(
        ...
        if include_sprav:
            ...
            for col in ("J", "K", "L", "M"):
                c = ws[f"{col}{sprav_start}"]
                c.border = (
                    c.border.copy(bottom=thin) if c.border else Border(bottom=thin)
                )
...
def _render_header(ws: Worksheet, body: Any) -> None:
    ...
    # Row 8: category (B8:F8 merged, thick outer border)
    ...
    apply_thick_outer_border(ws, "B8", "F8")
    ...
    dt_calc = _parse_date_ddmmyyyy(calc_date)
    set_cell(
        ws, "B10", dt_calc, bold=True, size=13, h="center", v="center", num_fmt=DATE_FMT
    )
